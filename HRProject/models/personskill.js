    ...
    try {
        const results = findRecordByPersonSkill(data).then(async foundRecord => {
            if (foundRecord == undefined) {
                var sql = `INSERT INTO personskill(person_personid, skill_skillid) VALUES('${data.personid}', '${data.skillid}')`;
...
const findRecordByPerson = async(personid) => {
    var sql = `
            SELECT * FROM personskill WHERE person_personid = ${personid}
            `;
    const results = await pool
        .query(sql)
        .then((data) => {
...
const findRecordByPersonSkill = async(data) => {
    var sql = `
            SELECT * FROM personskill WHERE (person_personid = ${data.personid} AND skill_skillid = ${data.skillid})
            `;
    const results = await pool
        .query(sql)
        .then((data) => {
